...
function delete_subm_pods() {
    use_kube_context $1
    ns=$2
    if kubectl wait --for=condition=Ready pods -l app=submariner-engine -n $ns --timeout=60s > /dev/null 2>&1; then
        echo Removing submariner engine pods...
        kubectl delete pods -n submariner -l app=submariner-engine
    fi
    if kubectl wait --for=condition=Ready pods -l app=submariner-routeagent -n $ns --timeout=60s > /dev/null 2>&1; then
