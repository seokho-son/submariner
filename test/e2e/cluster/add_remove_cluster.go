...
const clusterManagementTimeoutMultiplier = 3
...
var _ = PDescribe("[expansion] Test expanding/shrinking an existing cluster fleet", func() {
	...
	It("Should be able to add and remove third cluster", func() {
		...
		tcp.RunNoConnectivityTest(f, false, framework.NonGatewayNode, framework.GatewayNode, framework.ClusterC, framework.ClusterA, clusterManagementTimeoutMultiplier)

		By(fmt.Sprintf("Verifying that a pod in cluster %q cannot connect to a service in cluster %q", clusterBName, clusterCName))
		tcp.RunNoConnectivityTest(f, true, framework.NonGatewayNode, framework.NonGatewayNode, framework.ClusterC, framework.ClusterB, clusterManagementTimeoutMultiplier)
		...
		tcp.RunConnectivityTest(f, false, framework.PodNetworking, framework.GatewayNode, framework.GatewayNode, framework.ClusterC, framework.ClusterB, clusterManagementTimeoutMultiplier)
		tcp.RunConnectivityTest(f, true, framework.PodNetworking, framework.NonGatewayNode, framework.NonGatewayNode, framework.ClusterC, framework.ClusterA, clusterManagementTimeoutMultiplier)
